...
def run():
    ...
    global my_chosen_pokemon, rounds, user_score, computer_score, draws, badge_name, gym_badge, img, badge_canvass, \
        session_pokemon
    rounds += 1
    print('\n\nRound:{}'.format(rounds))
    time.sleep(1)
    ...
    if not os.path.exists("battle.mp3"):
        urllib.request.urlretrieve(
            "https://vgmsite.com/soundtracks/pokemon-game-boy-pok-mon-sound-complete-set-play-cd/yzmctgipnq/"
            "1-15.%20Battle%20%28Vs.%20Trainer%29.mp3", "battle.mp3")
    ...
    if my_stat > opponent_stat:
        if not os.path.exists("victory.mp3"):
            urllib.request.urlretrieve(
                "https://vgmsite.com/soundtracks/pokemon-game-boy-pok-mon-sound-complete-set-play-cd/vqnnuvhutx/"
                "1-29.%20Victory%20%28Vs.%20Gym%20Leader%29.mp3",
                "victory.mp3")
        ...
        if not os.path.exists("loss.mp3"):
            urllib.request.urlretrieve(
                "https://vgmsite.com/soundtracks/pokemon-game-boy-pok-mon-sound-complete-set-play-cd/cmyqgmwfdc/"
                "1-20.%20Pok%C3%A9mon%20Gym.mp3",
                "loss.mp3")
        ...
        if not os.path.exists("draw.mp3"):
            urllib.request.urlretrieve(
                "https://vgmsite.com/soundtracks/pokemon-game-boy-pok-mon-sound-complete-set-play-cd/hjdhsxlpgp/"
                "1-33.%20Celadon%20City.mp3",
                "draw.mp3")
...
if start == 'yes':
    print("Let's battle some Pokemon!")     # music from https://downloads.khinsider.com
    if not os.path.exists("opening.mp3"):
        urllib.request.urlretrieve(
            "https://vgmsite.com/soundtracks/pokemon-game-boy-pok-mon-sound-complete-set-play-cd/vfywpihuos/"
            "1-01.%20Opening.mp3",
            "opening.mp3")
    mixer.init()
...
if play_wdp.lower() == 'yes':
    print('\n' * 2,
          "Great! You saw {} Pokemon this session, let's see if you can guess who's silhouette this is...\n\n\n"
          .format(len(session_pokemon)))
    opener = urllib.request.build_opener()
    # this code is because the website didn't like us scraping the file as python, we will look like a web-based program
    ...
    urllib.request.urlretrieve(
        "https://external-preview.redd.it/e5zoQw-hgw-LCjdhC_4G8IAcHxex5pzda_BD_FPTcBY.png?"
        "auto=webp&s=c0b96b5ec20010a15864b8a0c9b202c119e52fe8",
        "WDP background.png")
    ...
    while True:
        ...
        if wdp_answer.lower() == whos_that['name']:
            if not os.path.exists("correct.mp3"):
                urllib.request.urlretrieve(
                    "https://vgmsite.com/soundtracks/pokemon-red-blue-version-gb/wbuniqpomx/47%20Item%20Obtained.mp3",
                    "correct.mp3")
            mixer.init()
            ...
            if not os.path.exists("end.mp3"):
                urllib.request.urlretrieve(
                    "https://vgmsite.com/soundtracks/pokemon-red-blue-version-gb/lelaaewkcx/"
                    "52%20Pokedex%20Evaluation%20Fanfare.mp3",
                    "end.mp3")
            mixer.init()
